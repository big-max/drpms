...
//tabnav选项卡
  $(".tabnav li").each( function(i){ 
    ...
    $(this).click(function(){
				$(this).addClass("active").siblings("li").removeClass("active");
				var i=$(this).index();
      	$(".tabcont").html($(".tabnow"+i).html());
				 $('.tabcont .autoplay').slick({
						slidesToShow: 3,
						slidesToScroll: 1,
						autoplay: false,
						autoplaySpeed: 2000,
...
//D地图视图的导航hover效果
	...
	$(".treemenu > span").click(function(){
		...
		//$('.tabcontent').height( $(window).height() - $('.header').height() - 40);
...
　　});
  ...
  $(".btn-up").click(function(){
	...
	//自动化的信息显示隐藏
	  $(".swapcon").click(function(){
				$(this).find(".icon-chevron-down").toggleClass("icon-chevron-right");
				$(this).siblings(".form-horizontal").toggle();
				$(this).siblings(".twotit").toggle();
				$(this).siblings(".column").toggle();
			});
	//模块箭头
