...
import com.ibm.smartcloud.core.dao.hibernate.support.BeanUtils;
...
public class HibernateEntityDao<T, PK extends Serializable> extends HibernateDaoSupport implements IEntityDao<T, PK> {
	...
	public List<T> getAll(String orderBy, boolean isAsc) {
		...
		if (isAsc)
			return (List<T>) getHibernateTemplate().findByCriteria(DetachedCriteria.forClass(getEntityClass()).addOrder(Order.asc(orderBy)));
		else
			return (List<T>) getHibernateTemplate().findByCriteria(DetachedCriteria.forClass(getEntityClass()).addOrder(Order.desc(orderBy)));
	}
	...
	public void saveOrUpdateObj(Object entity) {
		getHibernateTemplate().saveOrUpdate(entity);
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.eigpay.dao.hibernate.extend.IEntityDao#save(java.lang.Object)
	 */
	...
	/**
	 * 创建Criteria对象，带条件和排序参数表
	 */

	@SuppressWarnings("unused")
	private Criteria createCriteria(List<Criterion> filterList, List<Order> orderList, boolean flag) {
		Criteria criteria = getSession().createCriteria(getEntityClass());
	...
	private Criteria createCriteria(Map<String, String> aliasMap, List<Criterion> filterList, List<Order> orderList, boolean flag) {
		Criteria criteria = getSession().createCriteria(getEntityClass());
	...
	public Page<T, PK> pagedQuery(Criteria criteria, int pageNo, int pageSize) {
		...
		// 执行查询
		Object reObject = criteria.setProjection(Projections.rowCount()).uniqueResult();
		int totalCount = (reObject == null ? 0 : (Integer) reObject);
		...
		// 如果查询的数量大于指定的最大值则进行缓存清理
		/*
		 * if(list!=null&&list.size()>maxCachedSize) {
		 * System.out.println("HiberbateEntityDao:Clear Cache");
	 ...
	 * @return 含总记录数和当前页数据的Page对象.
	 */
	public Page<T, PK> pagedQuery(int pageNo, Map<String, String> aliasMap, List<Criterion> filterList, List<Order> orderList, int pageSize) {
		Criteria criteria = createCriteria(aliasMap, filterList, orderList, true);
		return pagedQuery(criteria, pageNo, pageSize);
	}
	 ...
	 */
	@SuppressWarnings("unchecked")
	public PK getId(Class<T> entityClass, T entity) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException {
